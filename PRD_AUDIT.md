...
| Section 2's primary use case is "alert me when something material changes", but detection ended at a memo in a folder | Add Slack and SMTP delivery for notable findings only, best effort, with the outcome stored on the run record and `--notify-pending` to retry from records without reprocessing | src/service/notify.py, pipeline/runner.py |
| Narrative change detection was inert on a fresh install: baselines existed only for filings the poller had already processed, so every screened passage read `unestablished` until a company filed twice under a running system | Add `--backfill` to seed each company's most recent past filing, writing no memos and acknowledging nothing, plus a startup note naming companies still unseeded | src/pipeline/backfill.py, __main__.py |
| No structured record of a processed filing existed, so nothing could query past runs without re-running the pipeline or parsing memo prose | Write a versioned run record beside each memo after the memo is durable, and serve it through a read API with Markdown and PDF views | src/service/records.py, api.py, pdf.py |
...
301 offline tests pass. Tests cover fiscal/comparative mapping, unusable tags, revision cutoffs, total debt semantics, table scales/dates/missing cells, unsupported layouts, div/hidden prose, restatement false positives, memo provenance, end-to-end output, retries, failed output writes, duplicate suppression, company failure isolation, benchmark metrics, review exports, local review persistence/HTTP behavior, strict labeled-benchmark compilation, retrieval-profile selection, query/document prefix handling, cross-encoder ordering, stack-fingerprint enforcement, narrative screening and date normalization, new/revised/unchanged/unestablished classification, baseline durability and as-of lookup, memo severity separation, stability of the comparison under shifted chunk boundaries, peer alignment across differing fiscal calendars, run-record durability and versioning, API routing and error codes, PDF rendering including markup escaping, baseline seeding including its refusal to write memos or acknowledge accessions, and alert policy, message construction, transport error handling and retry-from-records. The test suite is pinned to the lexical stack so it needs no model download; one opt-in test loads real weights and is skipped by default.
...
| Judgment of changes | Baselines can now be seeded before the first poll, so change detection works from day one rather than after a quarter. Language rules establish change against the company's previous filing and label unchanged repeats routine. They still do not establish negation or materiality, and a passage reworded to the same meaning reads as new. Similarity thresholds and restatement materiality remain uncalibrated: no labeled sample of real filing-to-filing edits has been measured, so false-positive and false-negative rates are unknown. |
...
| Service/infrastructure | A FastAPI read API and reportlab PDF export now exist over structured run records, and the Docker image was rebuilt and verified. Still missing: a browser UI, PostgreSQL/Redis, a durable Qdrant server, LangSmith instrumentation and any cloud deployment. Polling remains CLI-owned and the API returns 501 for it, because triggering runs from a web worker needs the queue that guarantees a single writer. The API has no authentication. |
